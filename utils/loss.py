...
class Contrastive_Loss(nn.Module):
    ...
    def Contrastive_loss_calc(self, feature, pre_coord, label_id):
        # Obtain the feature value of the detection location
        ...
        #selected_feature.size() => [batch,length,num,channel]
...
class connected_loss(nn.Module):
    ...
    def delayed_predict_connection(self, vector, coord):
        ...
        for delay in range(1, 4):
            ...
            yield F.softmax(similar / 0.001, dim=-1), F.softmax(similar / 0.001, dim=-2)

    def forward(self, vector, label_id, coord):
        """Cross Entropy Loss function at t,t+1

        Args:
            vector (tensor[batch,length,num+1,dim]): vector of  model output
            label_id (tesor[batch,length,num]): label id
            coord (tesor[batch,length,num,2(x,y)]): label coordinate
